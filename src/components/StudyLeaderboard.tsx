...
import { publishWeeklyStudy, fetchWeeklyLeaderboard } from '@/lib/studyShare';
 ...
 */
export function StudyLeaderboard({ weekTotalMs, weekStart, delay = 0.25 }: Props) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (!mine) {
        rows.push({ userId: authUser.id, displayName, weekStart, totalMs: weekTotalMs, avatarUrl: myAvatar });
      } else {
  ...
  }, [authUser, settings?.name, friendIds, weekStart, weekTotalMs]);
